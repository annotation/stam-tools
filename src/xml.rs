...
impl<'a> XmlConversionConfig<'a> {
    ...
    fn element_config(&self, node: Node) -> Option<&XmlElementConfig> {
        ...
        for elementconfig in self.elements.iter().rev() {
            if elementconfig.expression.test(&nodepath, self) {
    ...
    pub fn iter<'a>(
        ...
        self.0.trim_start_matches('/').split("/").map(|segment| {
            if let Some((prefix, name)) = segment.split_once(":") {
                if let Some(namespace) = config.prefix_to_namespace.get(prefix).map(|x| x.as_str())
                {
                    (Some(namespace), name)
                } else {
                    panic!(
...
impl<'a> Default for XmlConversionConfig<'a> {
    fn default() -> Self {
        Self::new()
            .with_prefix("xml", NS_XML)
            .with_prefix("xsd", "http://www.w3.org/2001/XMLSchema")
            .with_prefix("xhtml", NS_XHTML)
            .with_prefix("html", NS_XHTML)
            ...
            .with_attribute("*", "", "*", |a| a)
            .with_attribute("*", NS_XML, "id", |a| {
                a.with_handling(AttributeHandling::Identifier)
            })
            .with_attribute("*", NS_XML, "space", |a| {
                a.with_handling(AttributeHandling::Exclude) //already handled by text extractor, no need to turn into an annotation
            })
            .with_attribute("//html:meta", "", "name", |a| {
                a.with_handling(AttributeHandling::Key)
            })
            .with_attribute("//html:meta", "", "content", |a| {
                a.with_handling(AttributeHandling::Value)
            })
            .with_element("*", |e| e)
            .with_element("/html:html/html:head", |e| {
                e.with_handling(ElementHandling::PassThrough)
            })
            .with_element("/html:html/html:head/*", |e| {
                e.with_handling(ElementHandling::Exclude)
            })
            .with_element("/html:html/html:head/html:title", |e| {
                e.with_handling(ElementHandling::AnnotateResourceWithTextAsData)
            })
            .with_element("/html:html/html:head/html:meta", |e| {
                e.with_handling(ElementHandling::AnnotateResource)
            })
            ...
            .with_element("//html:li/html:li", |e| {
                e.with_textprefix("  * ").with_textsuffix("\n")
            })
            .with_element("//html:li/html:li/html:li", |e| {
                e.with_textprefix("    * ").with_textsuffix("\n")
            })
            .with_element("//tei:br", |e| e.with_textsuffix("\n"))
            .with_element("//tei:p", |e| e.with_textprefix("\n").with_textsuffix("\n"))
            .with_element("//tei:item", |e| e.with_textprefix("* "))
            .with_element("//tei:item", |e| e.with_textsuffix("\n"))
            .with_inject_dtd(HTML_ENTITIES_DTD)
    }
}
...
pub fn from_xml<'a>(
    ...
    if xmlstring[..100].find("<!DOCTYPE").is_none() {
        if let Some(dtd) = config.inject_dtd {
...
struct XmlToStamConverter<'a> {
    ...
    config: &'a XmlConversionConfig<'a>,
}
